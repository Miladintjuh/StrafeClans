...
public sealed class StrafeAttempt : INotifyPropertyChanged
{
    ...
    public bool StartsSessionBlock { get; set; }
    ...
    public double? TotalTimeFromReleaseMs => ClickTimeMs.HasValue ? ClickTimeMs.Value - ReleaseTimeMs : null;
    public TimingGrade Grade { get; set; } = TimingGrade.Unrated;
    public string Diagnosis { get; set; } = "Waiting for shot";
    public string CleanTimingLabel { get; set; } = string.Empty;
    public string MovingShotTimingLabel { get; set; } = string.Empty;
    ...
    public string DirectionSymbol => (FromKey, ToKey) switch
    {
        ("A", "D") => "->",
        ("D", "A") => "<-",
        _ => $"{FromKey}->{ToKey}"
    ...
    public string GradeLabel => Grade switch
    {
        ...
        TimingGrade.Late => "Late counter/click",
        TimingGrade.EarlyClick => "Early click",
    ...
    public string MistakeLabel
    {
        get
        {
            ...
            if (Grade == TimingGrade.Late) mistakes.Add(ClickFromCounterMs.HasValue && ClickFromCounterMs.Value > 160 ? "Late click" : "Late counter");
            if (Grade == TimingGrade.EarlyClick) mistakes.Add("Early click");
            if (IsMovingAtClick) mistakes.Add("Moving shot");
            return mistakes.Count == 0 ? "-" : string.Join(", ", mistakes);
        }
    }
    ...
    public string TotalTimeLabel => TotalTimeFromReleaseMs.HasValue ? $"{TotalTimeFromReleaseMs.Value:+0.0;-0.0;0.0} ms" : "no click";
    public int TracePoints => MouseTrace.Count;
    public string TracePointsLabel => TracePoints == 0 ? "-" : TracePoints.ToString();
    public string TraceLabel => TracePoints == 0 ? "×" : "✓";
    public string TableWhatHappened
    {
        get
        {
            var items = new List<string>();
            string diagnosis = Diagnosis.ToLowerInvariant();
            ...
            if (diagnosis.Contains("tiny overlap")) Add("minor key overlap");
            else if (diagnosis.Contains("overlap")) Add("key overlap");

            if (diagnosis.Contains("late:")) Add("late counter");
            if (diagnosis.Contains("slow shot")) Add("slow click");

            if (IsMovingAtClick)
            {
                Add(ResolveMovingShotTimingLabel());
            }
            else if (diagnosis.Contains("shot too early") || Grade == TimingGrade.EarlyClick)
            {
                Add("early click");
            }

            if (!HasClick) Add("no shot");

            if (items.Count == 0 && Grade == TimingGrade.Overlap) Add("key overlap");
            if (items.Count == 0 && Grade == TimingGrade.Late) Add("late timing");
            if (items.Count == 0 && Grade == TimingGrade.Perfect) return ResolveCleanTimingLabel() + ".";
            if (items.Count == 0) return "clean.";
            return string.Join(" and ", items) + ".";
        }
    }

    private string ResolveCleanTimingLabel()
    {
        if (!string.IsNullOrWhiteSpace(CleanTimingLabel)) return CleanTimingLabel;
        if (!TotalTimeFromReleaseMs.HasValue) return "clean";
        double total = TotalTimeFromReleaseMs.Value;
        if (total <= 90) return "fast clean";
        if (total <= 145) return "perfect clean";
        if (total >= 190) return "just in time clean";
        return "controlled clean";
    }

    private string ResolveMovingShotTimingLabel()
    {
        if (!string.IsNullOrWhiteSpace(MovingShotTimingLabel)) return MovingShotTimingLabel;
        if (ClickTimeMs.HasValue && ClickTimeMs.Value < OppositeDownTimeMs) return "shot too soon";
